...
!  to a one line.
!  ifort -o ASpatial.exe -O3 area_2spatial.f90
!
...
! 11/08/2017  using iner-domain grid localiza.csv
!
module land
    ...
    integer,allocatable :: idcg(:)      ! total grids
...
subroutine lee_localiza
    implicit none
    integer nx,ny,k,ncel
    character(len=10)cdum
    open(unit=32,file='localiza.csv',status='old')
    read (32,*) cdum  !Header
    read (32,*) nx,ny  !Header
    ncel=nx*ny
    print *,ncel
    allocate(idcg(ncel))
    do k=1,ncel
